...
const uri =
  "mongodb+srv://ysotocastro:Mnavarro3041.@cluster0.h5cyaao.mongodb.net/?retryWrites=true&w=majority";
...
router.delete("/eliminar-tarea/:id/", async (req, res) => {
  ...
  try {
    ...
    const db = client.db("Tasklists");
    const collection = db.collection("Tasks");
    const newDocument = await collection.deleteOne({
      _id: { $eq: IdSeleccionado },
    });
    client.close();
    res.status(200).send(newDocument);
  } catch (error) {
